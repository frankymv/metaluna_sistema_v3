...
//ventana para agregar cantidad de producto
    ...
    public function store(){
        ...
        //dd($this->id_forma_pago);

       if($this->id_forma_pago==="CREDI" ) {
            $data->fill([
                'credi'=>true,
                'total_credito'=>$this->sub_total,
                'fecha_limite_credito'=>$this->fecha_venta,
                'observaciones_credito'=>$this->observaciones_venta,
                'anticipo_v'=>$total_abono_anticipado,
                'nuevo_saldo_v'=> ($total_cre-$total_abo)+$this->sub_total,
                'saldo_anterior_v'=> $total_cre-$total_abo,
                //'total_v'=>($this->sub_total+$total_cre)-$total_abo
            ]);

            if(($total_cre-$total_abo)+$this->nuevo_saldo>=$cliente->limite_credito && !$this->autorizacion_limite_credito){
...
/*
            $this->dispatch(
                'venta-completada',
                pdf: route('pdfVentaRapida', $data->id),
...
*/
    ...
    public function liberarCredito(){
        ...
        //User::where('email',$this->email_edit)->wherePassword(Hash::make($this->codigo_edit))->first();
    ...
    public function exportarVentaRapida($id)
        {
            ...
            $pdf = PDF::loadView('/livewire/pdf/fila/VentaRapida',['venta' => $venta,'cliente'=>$cliente]);
