...
final class ViaticoTable extends PowerGridComponent
{
    public string $tableName = 'viatico-table-ufebt0-table';
    ...
    {
        return Viatico::query()->with('User');
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('no_viatico')
            //->add('user_id')
            ->add('user_id', fn (Viatico $model) => $model->User?->nombres ?? 'N/A')
            ->add('fecha_viatico_formatted', fn (Viatico $model) => Carbon::parse($model->fecha_viatico)->format('d/m/Y'))
            ->add('total_viatico')
            ->add('observaciones')
            ->add('created_at');
    }
    ...
    {
        return [
            Filter::datepicker('fecha_viatico'),
        ];
    }
    ...
    // Ordenamiento
        ...
        $datas = $query->get();
        return exportarGeneralPDF('Viatico', [
            'data' => $datas,
        ]);
    }
    ...
    public function actions(Viatico $row): array
    {
        return [
            Button::add('edit')
                ->icon('default-edit')
