...
////////////////////////////PDF//////////////////////////
   public function cancel(){
    ...
    public function exportarGeneral($id)
    {
        ...
        $pdf = Pdf::loadView('/livewire/pdf/pdfCotizacion',['venta' => $venta,'cliente'=>$cliente,'saldo_anterior'=>$saldo_anterior,'saldo_actual'=>$saldo_actual,'abono'=>$abono]);
    ...
    public function pdfCotizacionn($id)
    {
        $fecha_reporte=Carbon::now()->toDateTimeString();
        $saldo_actual=0;
        $saldo_anterior=0;

        $venta=Cotizacion::with('productos')->where('no_venta',$id)->get()->first()->toArray();
        ...
        $pdf = PDF::loadView('/livewire/pdf/pdfCotizacion',['venta' => $venta,'cliente'=>$cliente,'saldo_actual'=>$saldo_actual,]);
    ...
    public function pdfCotizacion($id)
    {
        $venta=Cotizacion::with('productos')->where('no_venta',$id)->get()->first()->toArray();
        $cliente=Cliente::find($venta['cliente_id'])->toArray();
        $pdf = PDF::loadView('/livewire/pdf/fila/Cotizacion',['venta' => $venta,'cliente'=>$cliente]);
        return $pdf->stream('Cotizacion.pdf',array("Attachment" => false));
    }
