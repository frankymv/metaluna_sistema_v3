<x-frk.components.template-index>
    ...
    <x-slot:body>
        <section class="container mx-auto px-2 lg:px-6 space-y-6">
            ...
            <!-- CABECERA -->
            <div class="rounded-xl shadow bg-gray-400 py-2 px-5">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div class="lg:col-span-2 bg-white rounded-xl shadow p-4 space-y-3">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-4 gap-2">
                        <x-frk.components.label-input label="Cod. Inter"  wire:model="codigo_interno" />
                        <x-frk.components.label-input label="Cod. Mayor"  wire:model="codigo_mayorista" />
                        <x-frk.components.label-input label="NIT"  wire:model="nit" />
                        <x-frk.components.label-input label="Tipo Cliente"  wire:model="tipo_cliente" />
                    </div>

                    <x-frk.components.label-input-horizontal label="Nombre" error="nombres_cliente"  wire:model="nombres_cliente" />
                        ...
                        <tbody>
                            @foreach($productosDetalle as $key => $value)
                                <tr class="border-b">
                                    ...
                                    <td class="px-3 py-2 text-right">Q {{ $value['precio_final'] }}</td>
                                    <td class="px-3 py-2 text-right font-semibold">Q {{ $value['subtotal_producto'] }}</td>
                                </tr>
                            @endforeach
                            ...
                            <tr class="bg-gray-100 font-semibold">
                                ...
                                <td class="text-right">Q {{ $sub_total }}</td>
                                <td></td>
                            </tr>
                        </tbody>
                    </table>
            ...
            <x-frk.components.button color="gray" label="Finalizar Cotizacion" wire:click="store()" />
            <!-- CREDITO -->
    ...
    <x-slot:footer>
        @if($isSearchCliente) @include('livewire.pages.cotizacion.searchCliente') @endif
        @if($isAddProduct) @include('livewire.pages.cotizacion.addProduct') @endif
        @if($isSearchProduct) @include('livewire.pages.cotizacion.searchProduct') @endif
        @if($isDetalleVenta) @include('livewire.pages.cotizacion.detalleVenta') @endif
        @if($isPrintVenta) @include('livewire.pages.cotizacion.printVenta') @endif
    </x-slot:footer>
