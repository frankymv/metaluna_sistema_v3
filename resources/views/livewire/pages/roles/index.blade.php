<x-frk.components.template-index>
                    ...
                    <tbody class="bg-white">
                        ...
                        <tr class="text-gray-700">
                            ...
                            <td class="px-4 py-3 text-sm border">
                                @foreach ($data->permissions as $roles )
                                    {{$roles->name}},
                                @endforeach
                            ...
                            <td class="px-4 py-3 text-sm border">
                                <div class="flex">
                                    ...
                                    <x-frk.components.button-icon color="orange" icon="fa-solid fa-eye" wire:click="show({{$data->id}})" />
    ...
    <x-slot:footer>
        @if($isCreate)
            @include('livewire.pages.roles.create')
        @endif
        @if($isEdit)
            @include('livewire.pages.roles.edit')
        @endif
        @if($isShow)
            @include('livewire.pages.roles.show')
        @endif
        @if($isDelete)
            @include('livewire.pages.roles.delete')
        @endif
    </x-slot:footer>
</x-frk.components.template-index>
