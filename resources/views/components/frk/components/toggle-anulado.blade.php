...
<div class="w-full flex-wrap rounded-lg mx-1 px-1 pb-1" x-data="{open: @entangle('anulado')}" >
    ...
    @endif
        ...
        <span class="font-semibold text-xs mr-1">Si</span>

        <div class="rounded-full w-8 h-4 p-0.5"
            :class="{'bg-green-500': open === 'true', 'bg-red-500': open === 'false'}">

            <div class="rounded-full w-3 h-3 bg-white transform mx-auto duration-300 ease-in-out"
                :class="{'-translate-x-2': open === 'true', 'translate-x-2': open === 'false'}">
            </div>
        </div>

        <span class="font-semibold text-xs ml-1">No</span>
    </div>
</div>
